function exibirMoedas() {
    // gera números aleatorios, entre 1 e 5, para cada moeda
    ...
    // chama o metodo criar moedas passando os argumentos
...
function criarMoedas(num, pai, moeda, textoAlt, classe) {
    // cria um laço de repetição para inserir várias imagens de moedas na pagina
    for (var i = 1; i <= num; i++) {
...
function conferirSoma() {
    ...
    if (soma == 0 || isNaN(soma)) {
        alert("Informe o valor da soma (use pronto para separar decimais)");
        inSoma.focus();
        return;
    }
